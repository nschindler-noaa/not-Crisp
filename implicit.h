...
public:
  ...
  void notify()
    {
        /* NOTE: this loop is implemented "weirdly" because call()ing
         *  one of the notification functions may result in that
         *  object being removed from the list.
         */
        utImplicitBaseItr itr(*this);
        utMaplet0 *m, *ml;
        m = (utMaplet0*) itr.next();
        while (m) {
            ml = m;
            m = (utMaplet0*) itr.next();
            ml->call();
