 ...
 *   	crisp run in realtime mode. (NOT part of COMPASS batch or GUI! See \ref crisp_calib)
 */
...
#include "crisp_extract.h"

/* int crisp_extract(req, rslt) - perform an extraction of a data vector
 * from a recent crisp realtime mode run.  The format of the datafile
 * extracted from must be that of a "realtime.real" file.  This
 * routine takes pointers to CrRequest and CrResult structs.  The
 ...
 * datafile, function "crisp_extract_cleanup(req)" must be called, which
 * will close the file.
 *
 * The extraction is placed in the CrResult struct.  The "vec" field
    ...
    CRResult *rslt;
{
	...
	if (!strcmp(str1, "Release")) {
	    str1[0] = 0; str2[0] = 0;
	    if (2 != sscanf(input_buffer, "%*s%s%*s%*s%*s%s", str1, str2))
		return CR_BADTOKEN;
	    if (!strcmp(str1, req->release_site) && !strcmp(str2, stock))
		found = 1;
	}
    }
